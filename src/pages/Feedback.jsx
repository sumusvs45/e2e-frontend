...
const Feedback = ({ token }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await fetch('https://e2e-backend.onrender.com/api/v1/feedback', {
      method: 'POST',
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-gray-50 min-h-screen py-6 px-4">
      ...
      <Link to="/"
        ...
        className="w-full max-w-md py-3 px-4  text-blue-500 font-semibold rounded-lg hover:text-black  focus:outline-none focus:ring-2 focus:ring-yellow-400 transition"
      >
        ContinueShopping
      </Link>
    </div>
